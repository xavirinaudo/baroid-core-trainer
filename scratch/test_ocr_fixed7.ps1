...
function Await($asyncOperation, $type) {
    $interfaces = $asyncOperation.GetType().GetInterfaces()
    $isProgress = $interfaces | Where-Object { $_.Name -like 'IAsyncOperationWithProgress`2' }
    
    if ($isProgress) {
        $asTask = [System.WindowsRuntimeSystemExtensions].GetMethods() | 
            Where-Object { $_.Name -eq 'AsTask' -and $_.GetGenericArguments().Count -eq 2 -and $_.GetParameters().Count -eq 1 }
        if ($asTask.Count -gt 1) { $asTask = $asTask[0] }
        
        $iface = $interfaces | Where-Object { $_.Name -like 'IAsyncOperationWithProgress`2' }
        $tProgress = $iface.GetGenericArguments()[1]
        
        $genericAsTask = $asTask.MakeGenericMethod($type, $tProgress)
    } else {
        $asTask = [System.WindowsRuntimeSystemExtensions].GetMethods() | 
